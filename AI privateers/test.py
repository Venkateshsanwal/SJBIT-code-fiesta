...
from pdf_creator import save_pdf_to_database
import pandas as pd
import base64, random
import time, datetime
from jd import job_role
from pyresparser import ResumeParser
from app1 import job_decription_filter
...
def run():
    ...
    activities = ["Applicant", "Hiring Manager","H.R"]
    ...
    table_sql = "CREATE TABLE IF NOT EXISTS " + DB_table_name + """
                    (ID INT NOT NULL AUTO_INCREMENT,
                     ...
                     Predicted_Field VARCHAR(50)  NULL,
    ...
    if choice == 'Applicant':
        ...
        jd=st.selectbox("Check the Job Role:",["None","Data Science","IOS Development","UI/UX Development","Android Development","Web Development"])
        emp=st.empty()
        if jd=="Data Science":
            emp.write(job_role[0])
        elif jd=="IOS Development":
            emp.write(job_role[3])
        elif jd=="UI/UX Development":
            emp.write(job_role[4])
        elif jd=="Android Development":
            emp.write(job_role[2])
        elif jd=="None":
            emp.write("Select a Job Role.")
        else:
            emp.write(job_role[1])
...
# Join us at BioSymphony and contribute to revolutionizing the field of computational biology and drug discovery through innovative machine-learning solutions!''')
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                # name_candidate=resume_data["name"]
                # selected_row=resume_scores[resume_scores["name"]==name_candidate]
                # if not selected_row.empty:
    # Retrieve the score for the specified name
                # score_for_name = selected_row['scores'].iloc[0]
                #     # st.write(score_for_name)
                # resume_score=int(score_for_name)
                #nAM
                resume_score = 0
        ...
        ## Admin Side
        st.success("Welcome to Hiring Manager's Dashboard")
        ...
        if st.button('Login'):
            if ad_user == 'ai' and ad_password == 'ai':
                ...
                # if st.button("Update Job Designation"):
                
                # designation = st.text_input("Enter the new job designation:")
                job_description=st.text_input("Enter the job description.")
                save_pdf_to_database(text=job_description)                    
                
                # resume_scores1=job_decription_filter(reco_field)
                # sc= resume_scores1['scores']
                # st.write(sc)
                ...
                #         delete_button_key = f"delete_button_{row['ID']}"  # Ensure unique keys
                #         if st.button(f"Delete Row {row['ID']}", key=delete_button_key):
                #             delete_row(row['ID'])
